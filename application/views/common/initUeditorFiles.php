<script type="text/javascript">
    ...
    <?=$container?>_editor.ready(function () {
        ...
        //_editor.hide();
        //侦听文件上传
        <?=$container?>_editor.addListener('afterUpfile', function (t, arg) {
            console.log(arg[0]);
            $("#<?=$container?>file").attr("value", <?=$container?>_editor.options.filePath + arg[0].url);
        })
    });
    //弹出文件上传的对话框
    $("#<?=$container?>").click(function(){
        var myFiles = <?=$container?>_editor.getDialog("attachment");
        myFiles.open();
    });
</script>